...
import vertexai.generative_models as genai_vertex
...
class GeminiModel(abc.ABC):
    def __init__(self, version: str | GeminiVersion, typing_module):
        self._version = version
        self._typing_module = typing_module

    @abc.abstractmethod
    async def _api_generate(self, model_name, request_contents, request_cfg):
        pass
    ...
    async def generate(
        ...
        reqest_contents = self._build_messages(request)
        ...
        response = await self._api_generate(
            self._version, reqest_contents, request_cfg
        )

        self._respond(response, ctx)
    ...
    def _build_messages(self, request: GenerateRequest) -> list[genai.Content]:
        ...
        for msg in request.messages:
            ...
            reqest_contents.append(
                self._typing_module.Content(parts=content_parts, role=msg.role)
            )
...
class GeminiGoogleAi(GeminiModel):
    def __init__(self, client: genai.Client, version: str | GeminiVersion):
        super.__init__(version, genai.types)
        self._client = client

    async def _api_generate(
        self, model_name, request_contents, request_cfg
    ) -> GenerateResponse:
        return await self._client.aio.models.generate_content(
            model=self._name, contents=request_contents, config=request_cfg
        )


class GeminiVertex(GeminiModel):
    ...
    def __init__(self, version: str | GeminiVersion):
        ...
        """
        super.__init__(version, genai_vertex)

    async def _api_generate(
        self, request_contents, request_cfg
    ) -> GenerateResponse:
        return await genai_vertex.GenerativeModel(
            self._version
